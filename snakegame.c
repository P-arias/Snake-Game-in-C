    ...
    while(1){ 
        ...
        switch(input){
            ...
            case KEY_UP:
                xDir = 0;
                yDir = -1;
                set_ticker(speed + (speed*.6));//prevents snake from going faster vertically than it does horizontally
                break;
            case KEY_DOWN:
                xDir = 0;
                yDir = 1;
...
Pieces are inserted at the tail of the list. snakeCount is incresed and subtracted from the 
intial speed, the time interval of alarms is reduced thus the snake goes faster
*/
void grow(){
...
/*Author: Pedro Arias 
Function frees memory allocated for each node of linked List for the snake pieces.
*/
void deleteList(){
